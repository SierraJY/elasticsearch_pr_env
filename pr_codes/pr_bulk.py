...
# 강제로 색인 refresh
# 이 코드는 Elasticsearch에서 bulk insert 후 색인을 즉시 검색 가능하게 만드는 코드입니다.
# Elasticsearch는 성능을 위해 기본적으로 색인을 비동기로 처리하는데,
# refresh를 호출하면 지금까지 삽입된 문서들을 즉시 검색 가능하도록 강제로 색인을 갱신합니다.
...
try:
    with open("pr_data/ecommerce.json", "r", encoding="utf-8") as f:
...
try:
    # TODO: ID가 5인 문서를 조회
    doc = es.get(index="ecommerce", id="VhUTQpUBT_-Ind6MuiLS")  
